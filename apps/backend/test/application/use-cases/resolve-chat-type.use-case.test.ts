  ...
  describe('audit logging error handling', () => {
    it('should catch and log audit log errors without throwing', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Error logging audit for chat type resolution',
        auditError,
        {
          userId: mockAuditContext.userId,
        }
    ...
    it('should continue execution when audit log fails for successful resolution', async () => {
      const param = uuidv7()
      const resolvedId = uuidv7()
      const auditError = new Error('Audit database down')
    ...
    it('should continue execution when audit log fails for not found case', async () => {
      const param = 'missing-chat'
      const auditError = new Error('Audit service timeout')
    ...
    it('should handle audit log throwing non-Error objects', async () => {
      const param = 'test-param'
      const resolvedId = uuidv7()
