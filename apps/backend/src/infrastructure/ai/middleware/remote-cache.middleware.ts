  ...
  wrapGenerate: async ({ doGenerate, params }) => {
    ...
    if (redis) {
      try {
        ...
        if (raw !== null) {
          ...
          console.log('[cache-middleware] wrapGenerate: CACHE HIT — returning cached response')
          return {
            ...cached,
            response: {
              ...cached.response,
        ...
        console.log('[cache-middleware] wrapGenerate: CACHE MISS — calling model')
      } catch (error) {
    ...
    if (redis) {
      try {
        ...
        console.log('[cache-middleware] wrapGenerate: response WRITTEN to cache')
      } catch (error) {
    ...
    if (redis) {
      try {
        ...
        if (raw !== null) {
          ...
          console.log('[cache-middleware] wrapStream: CACHE HIT — returning simulated stream')
        ...
        console.log('[cache-middleware] wrapStream: CACHE MISS — calling model')
      } catch (error) {
      ...
      async flush() {
        ...
        if (redis) {
          try {
            ...
            await redis.set(cacheKey, compressed, { ex: 3600 })
            console.log('[cache-middleware] wrapStream: response WRITTEN to cache')
          } catch (error) {
