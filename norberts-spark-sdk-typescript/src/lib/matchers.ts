...
import { NorbertsSparkSDKDefaultError } from '../models/errors/norberts-spark-sdk-default-error.js'
...
): MatchFunc<T, E | NorbertsSparkSDKDefaultError | ResponseValidationError> {
  return async function matchFunc(
    response: Response,
    request: Request,
    options?: { resultKey?: string; extraFields?: Record<string, unknown> }
  ): Promise<
    [result: Result<T, E | NorbertsSparkSDKDefaultError | ResponseValidationError>, raw: unknown]
  > {
    let raw: unknown
    ...
    if (!matcher) {
      return [
        {
          ok: false,
          error: new NorbertsSparkSDKDefaultError('Unexpected Status or Content-Type', {
            response,
            request,
    ...
    if (matcher.enc === 'fail') {
      return [
        {
          ok: false,
          error: new NorbertsSparkSDKDefaultError('API error occurred', {
            request,
            response,
            body,
          }),
